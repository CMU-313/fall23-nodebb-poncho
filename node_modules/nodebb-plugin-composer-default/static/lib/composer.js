...
define('composer', [
	...
	composer.enhance = function (postContainer, post_uuid, postData) {
		...
		*/
		...
		// ******************************** START ANONYMOUS FEATURE ******************************** //
		// ***************************************************************************************** //
		// ***************************************************************************************** //
		
		// helper function to remove all instances of one array from another array

		// finds all buttons and onclick events for them
		var anonymousBtn = postContainer.find('.composer-anonymousSubmit');
		var anonymousBtnStduent = postContainer.find('.composer-anonymousSubmitStudent');
		var anonymousBtnInstructor = postContainer.find('.composer-anonymousSubmitInstructor');

		// can only use up to 15 characters for each tag :(
		const anon_tags = ['anonymous', 'anon-student', 'anon-instructor'];

		// this function is triggered in composer.tpl as a button on click
		// it adds a tag named "anonymous" to the postData object
		// it then call tags.init in tags.js to reinitialize tag data.
		function OnAnonymousSubmit(e, i) {
			composer.posts[post_uuid].tags.push(anon_tags[i]);
			tags.init(postContainer, composer.posts[post_uuid]);
		}
		
		anonymousBtn.on('click', function (e) {
			OnAnonymousSubmit(e, 0);
		});

		anonymousBtnStduent.on('click', function (e) {
			OnAnonymousSubmit(e, 1);
		});

		anonymousBtnInstructor.on('click', function (e) {
			OnAnonymousSubmit(e, 2);
		});

		// *************************************************************************************** //
		// *************************************************************************************** //
		// ******************************** END ANONYMOUS FEATURE ******************************** //
	...
	async function createNewComposer(post_uuid) {
		...
		var data = {
			...
			submitOptions: [
				...
				// {
				// 	action: 'action:composer.discard',
				// 	text: 'Post Anonymously to Students',
				// },
				// {
				// 	action: 'post',
				// 	text: 'Post Anonymously to Instructors',
				// },
				// {
				// 	action: 'filter:composer.create',
				// 	text: 'Post Anonymously to All',
				// }
			],
		};
